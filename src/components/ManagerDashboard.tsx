        ...
        <div className="manager-dashboard">
            <header className="dashboard-header">
                <nav className="dashboard-nav">
                    ...
                    <button
                        className={`nav-link ${activeTab === 'leave-requests' ? 'active' : ''}`}
                ...
                <section className="leave-requests">
                    ...
                    {leaves.length === 0 ? (
                        ...
                        <table>
                            ...
                            <tbody>
                                {leaves.map(request => (
                                    <EmployeeFetcher key={request.id} employeeId={request.userId}>
                                        {({ employee, isLoading, isError }: { employee?: User; isLoading: boolean; isError: boolean }) => (
                                            <tr>
                                                <td>
                                                ...
                                                <td>
                                                    {request.status === 'pending' && (
                                                        <>
                                                            <button
                                                                onClick={() => handleLeaveDecision(request.id, 'approved', employee)}
                                                                disabled={isProcessing || isLoading || isError}
                                                            >
                                                                {isProcessing ? 'Processing...' : 'Approve'}
                                                            </button>
                                                            <button
                                                                onClick={() => handleLeaveDecision(request.id, 'rejected', employee)}
                                                                disabled={isProcessing || isLoading || isError}
                                                            >
